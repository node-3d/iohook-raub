...
HookProcessWorker::HookProcessWorker(Napi::Function &callback) :
Napi::AsyncProgressWorker<uiohook_event>(callback),
fHookExecution(nullptr)
{

}

void fillEventObject(uiohook_event event, Napi::Object out) {
	Napi::Env env = out.Env();
...
void HookProcessWorker::OnProgress(const uiohook_event * event, size_t size) {
	Napi::Env env = Env();
	uiohook_event ev;
	while (!zqueue.empty()) {
		ev = zqueue.front();
...
void HookProcessWorker::Execute(const Napi::AsyncProgressWorker<uiohook_event>::ExecutionProgress &progress) {
...
DBG_EXPORT JS_METHOD(debugEnable) { NAPI_ENV;
